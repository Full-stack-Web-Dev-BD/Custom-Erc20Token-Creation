...
contract("DappToken", async (accounts) => {
    ...
    it("transfer token ownership", async () => {
        ...
        assert.equal(log[0].args._from, accounts[0], 'logs the acount  the tokens are  fransferred from ')
        assert.equal(log[0].args._to, accounts[1], 'logs the acount  the tokens are  fransferred to')
